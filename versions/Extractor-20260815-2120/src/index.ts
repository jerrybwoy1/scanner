...
function targetPath(pathname: string): string | null {
  ...
  for (const prefix of ["/api/snapshot/","/api/batch/status/","/api/batch/control/","/api/batch/download/"]) {
    if (pathname.startsWith(prefix)) return pathname.replace("/api","");
  }
  return null;
}
...
async function proxy(request: Request, env: Env, url: URL) {
  ...
  const timeoutMs = path.startsWith("/batch/") ? 60_000 : path.startsWith("/snapshot") ? 45_000 : 190_000;
  ...
  try {
    ...
    outputHeaders.set("cache-control","no-store,max-age=0"); outputHeaders.delete("set-cookie");
